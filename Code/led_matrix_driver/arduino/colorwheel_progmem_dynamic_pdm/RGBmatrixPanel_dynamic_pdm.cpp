...
#include "RGBmatrixPanel_dynamic_pdm.h"
#include "gamma.h"
...
void RGBmatrixPanel::init(uint8_t rows, uint8_t a, uint8_t b, uint8_t c,
  ...
  noInterrupts();
  Serial.print("Initialing Ethernet stuff... ");
  Ethernet.begin(mac, ip);
  Udp.begin(localPort);
  // Ethernet.begin(matrix.mac, ip);
  // matrix.Udp.begin(matrix.localPort);
  Serial.println("Done");
  interrupts();
  ...
  int buffsize  = width * nRows, // x3 = 3 bytes holds 4 planes "packed"
  ...
  memset(packetBuffer, 17, sizeof(packetBuffer));
}
...
void RGBmatrixPanel::unpack(int row) {
  for(int bitPlane = 0; bitPlane < nPlanes; bitPlane++) {
    for (int col = 0; col < 32; col++) {
      byte color = packetBuffer[row * 32 + col] & 0x0F;
      byte COLOR = (packetBuffer[row * 32 + col] & 0xF0) >> 4;
      if(color == 1) color = 0;
      if(COLOR == 1) COLOR = 0;
      // Serial.println(ptr[row*32 + col]);
      // byte color = ptr[row * 32 + col] & 0x0F;
      // byte COLOR = (ptr[row * 32 + col] & 0xF0) >> 4;
      ...
      rowPixels[bitPlane * 32 + col] = 0;
      if (bitPlane < color) {
	rowPixels[bitPlane * 32 + col] |= B00011100;
      }
      if (bitPlane < COLOR) {
	rowPixels[bitPlane * 32 + col] |= B11100000;
      }
      if(recievedOnePacket) {
	Serial.print(rowPixels[bitPlane * 32 + col], HEX);
	Serial.print(", ");
      }
    }
  }
...
#define pew asm volatile(				\
 ...
 //  buffptr = ptr; //+= 32;
  ...
  //   // Serial.println(ptr[row*WIDTH + col]);
  //   // byte color = ptr[row * 32 + col] & 0x0F;
  //   // byte COLOR = (ptr[row * 32 + col] & 0xF0) >> 4;
  //   byte color = packetBuffer[row * 32 + col] & 0x0F;
  //   byte COLOR = (packetBuffer[row * 32 + col] & 0xF0) >> 4;
  ...
  if(plane == nPlanes - 1 && row == nRows - 1) {
    receiveUDPpacket();
  }
}

bool onlyLastPacket = 1;
bool unpack = 1;
void RGBmatrixPanel::receiveUDPpacket() {
  ...
  if (packetSize) {
    ...
    // recievedOnePacket = true;

    // if (unpack) {
    //   for (int y = 0; y < 16; y++) {
    // 	for (int x = 0; x < 32; x++) {
    // 	  byte color = packetBuffer[y * 32 + x] & 0x0F;
    // 	  byte COLOR = (packetBuffer[y * 32 + x] & 0xF0) >> 4;
