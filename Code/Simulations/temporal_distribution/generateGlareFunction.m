function glare = generateGlareFunction(luminance, angularResolution, angle, distance, glareLines)
%%


filterHalfRes = ceil(tan(deg2rad(angle))*distance);
if(mod(filterHalfRes, 2) == 1)
    filterHalfRes = filterHalfRes - 1;
end
...
if(glareLines)
    ...
    for i = 1:N-1
        for j = 1:N-1
            ...
            normalizedByArea = (2*pi*d + 2*pi)/(360*angleResolution);
            linesIndex = floor(angleDeg*angleResolution);
            if(linesIndex < 1)
                linesIndex = 1;
            end
            glareImg(i,j) = normalizedByArea * lines(linesIndex);
        end
    end
    
    glareImg = glareImg/sum(glareImg(:));
    % figure;
